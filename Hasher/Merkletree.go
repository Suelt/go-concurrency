...
func (m *MerkleTree) GetMerklePath(content Content, i int) ([][]byte, []int64, error) {
	current := m.Leafs[i]
...
func (m *MerkleTree) UpdateTree(cread Content, cwrite Content, j int) {

	merklePath, index, err := m.GetMerklePath(cread, j)
	if err != nil {
		log.Panicln(err)
	}
	...
	for i := 0; i < len(m.Leafs); i++ {
		b, err := m.Leafs[i].C.Equals(cread)
		config.UseRw(err)
		if b {
			a = i
			break
	...
	a = j
	hashcwrite, err := cwrite.CalculateHash()
	m.Leafs[a].C = cwrite
	m.Leafs[a].Hash = hashcwrite
	parent := m.Leafs[a].Parent
	for k := 0; k < len(merklePath); k++ {
		if index[k] == 1 {
			hashcwrite = append(hashcwrite, merklePath[k]...)

		} else {
			hashcwrite = append(merklePath[k], hashcwrite...)

		}

		hashcwrite, err = calHash(hashcwrite, m.hashStrategy)
		parent.Hash = hashcwrite
